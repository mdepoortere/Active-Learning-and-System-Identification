...
my_dat = StaticImageSet('/notebooks/data/static20892-3-14-preproc0.h5', 'images', 'responses')
...
dataset_config = dict(file='/notebooks/data/static20892-3-14-preproc0.h5',seed=5, batch_size=64, norm=True, cuda=True)
dataset_hash = make_hash(dataset_config)
dataset_entry = dict(dataset_loader="nnsetup.datamaker.create_dataloaders", dataset_config=dataset_config,
                     dataset_architect="Matthias Depoortere", dataset_comment=" Actively grown dataset")
Dataset().add_entry(**dataset_entry)

ens = []
for m_hash in model_hashes:
    restriction = ('config_hash in ("{}")'.format(m_hash),
                   'dataset_loader in ("{}")'.format("nnsetup.datamaker.create_dataloaders"),
                   'dataset_config_hash in ("{}")'.format(dataset_hash))
    TrainedModel().populate(*restriction)
    model = load_latest_model(dataset_config, model_config, dataset_hash, m_hash)
    ens.append(model)
